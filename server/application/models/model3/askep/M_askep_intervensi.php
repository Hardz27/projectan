...
class M_askep_intervensi extends CI_Model
{
    ...
    {
        $this->db->select('*');
        $this->db->from('notes_askep_intervensi');
        if ($id != null) {
            $this->db->where('id', $id);
        }
    ...
    {
        $this->db->select('*');
        $this->db->from('notes_askep_intervensi a');
        if ($id != null) {
    ...
    public function get_all($id='')
    {
        ...
        $this->db->group_by("d.askep_intervensi_kode");
        if($id!=''){
    ...
    public function get_render($visit,$no_rm){
        $query = $this->db->query("SELECT * FROM notes_askep_intervensi WHERE no_rm='$no_rm' AND id_pasien_visit='$visit'");
        return $query->result_array();
    }
    ...
    {
        return $this->db->insert('notes_askep_intervensi', $params);
    }

    public function edit($params,$where)
    {
        $this->db->where($where);
        $result = $this->db->update('notes_askep_intervensi', $params);
        return $result;
    }
