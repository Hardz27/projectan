...
class C_askep_intervensi extends MY_Controller
{

	function __construct()
	{
		parent::__construct();
		$this->load->model('rs/askep/m_askep_intervensi', 'askep');
	...
	public function intervensi_all_get(){ 
		$get = $this->get();
        // $this->response($get,200);
        if (empty($get['id'])) {
            $id='';
	...
	public function index_post(){
		...
		$data = [
			...
			'id_askep_diagnosa' 		=> $post['id_askep_diagnosa'], 
			'askep_intervensi_kode'       => $post['askep_intervensi_kode'],
