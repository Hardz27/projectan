...
class C_laporan_862 extends MY_controller
{
  // Parameter yang akan dipakain di endpoint ini
    ...
    {
        parent::__construct();
        $this->load->model('model3/global/M_order_produk_details', 'order_produk_details');
    }
  ...
  // Untuk validasi format tanggal di form validation
    ...
    // jika semua sdh oke, maka params akan diproses
        if ($this->form_validation->run() == true) {
            ...
            $get['type'] = 'racikan';
            $data = $this->order_produk_details->get($get);

            $total_biaya_racikan = 0;
            foreach ($data['result'] as $key => $value) {
                $total_biaya_racikan += $value->biaya_racikan;
            }

            $total_data = [
                'record_total'        => $data['record_total'],
                'record_filter'       => $data['record_filter'],
