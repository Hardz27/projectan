...
// Export ke pdf
   $rs_key = $this->session->userdata('rs_key');
     ...
     $response_pasien_reg = $this->_client_laporan->request('GET', 'statistik/icd10', [
      ...
      // $pdf->Cell(130,15, ' Klinik Bahagia', 1,1);
  ...
  { 
     ...
     $response_pasien_reg = $this->_client_laporan->request('GET', 'statistik/icd10', [
      ...
      for ($i=0; $i < count($keys); $i++) { 
        ...
        $sheet->setCellValue('D'. ($i + 10), $data[$keys[$i]][$tipe]['deskripsi']);
        $sheet->setCellValue('E'. ($i + 10), $total_pria);
        $sheet->setCellValue('F'. ($i + 10), $total_wanita);
        $sheet->setCellValue('G'. ($i + 10), '=SUM('.$total_pria.'+'.$total_wanita.')');
      }
