...
class C_laporan_statistik extends CI_Controller
{
  ...
  { 
      ...
      $sheet->getRowDimension('3')->setRowHeight(51);
      $sheet->mergeCells('A2:L3');
      $sheet->getStyle('A2:L3')->applyFromArray($titleStyle);
      ...
      $sheet->getStyle('A4:L4')->applyFromArray($subTitleStyle);
      $sheet->getStyle('A4')->getAlignment()->setHorizontal($horizontal_center);
      $sheet->getStyle('A4')->getAlignment()->setVertical($vertical_center);
      $sheet->setCellValue('A4', $clinic_profile['clinic_name']);

      $sheet->mergeCells('M2:N4');      
      $sheet->getStyle('M2')->getAlignment()->setHorizontal($horizontal_center);
      $sheet->getStyle('M2')->getAlignment()->setVertical($vertical_center);
      $sheet->setCellValue('M2', $kode);

      $sheet->getStyle('A6:D6')->getAlignment()->setVertical($vertical_center);
      $sheet->setCellValue('A6', 'Rekapitulasi ');
      $sheet->mergeCells('B6:D6');
      $sheet->setCellValue('B6', ': ' . date('d-M-Y',  strtotime($tgl_start)) .' - '.date('d-M-Y',  strtotime($tgl_end)));


      $cell = 8; //Mulai Bagian Body Laporan
