 ...
 *
 ****************************/
class C_notes_parkir_tiket extends CI_Controller {
  public function __construct() {
    ...
    $this->c_folder = "notes_parkir_tiket"; // <<< HANYA INI YANG PERLU DIRUBAH DI CONSTRUCT()!!!! SISANYA DIAMKAN
  ...
  function add_process() {
    $request_value = $this->_parkir_rs->request('POST', 'parkir_tiket', [
      'form_params' => [
        'entry_gate' => $this->input->post('entry_gate')
        ]
    ]);
  ...
  */
  ...
  function pdf() {
    // Export ke pdf
      ...
      $pdf->SetTitle('Notes Parkir Tiket '.$tgl_start.' - '.$tgl_end); // $pdf->SetTitle($title);
  ...
  */
  ...
  function excel() {
    ...
    $sheet->setCellValue('A1', 'Laporan Notes Parkir');            // $sheet->setCellValue('B3', $clinic_profile['clinic_name']);
