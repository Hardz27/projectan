 ...
 *
 **********************************************************************************/
class C_notes_askep_luaran extends CI_Controller
{
  ...
  public function detail_luaran($id){
    $response_code = $this->_client_rs->request('GET', 'askep_luaran_all',[
      'query' => [
        'id' => $id,
      ]
    ]);
  ...
  public function add($type)
  {
    ...
    if ($type == 'list') {
      ...
      $response_code = $this->_client_rs->request('GET', 'askep_luaran_all');
      $data['data_askep_luaran'] = json_decode($response_code->getBody()->getContents(), true)['data'];

      $response_code = $this->_client_rs->request('GET', 'askep_luaran/diagnosa');
  ...
  {
    $jlh_data = count($this->input->post('diagnosa_askep_luaran_kode'));
    ...
    for ($i = 0; $i < $jlh_data; $i++) {
      ...
      $params = [
        ...
        'askep_luaran_kode' => $this->input->post('diagnosa_askep_luaran_kode')[$i],
        // 'created_by' => $this->session->userdata['user_login']['id_user'],
        'json_data' => [
          'diagnosa_askep_luaran_nama' => $this->input->post('diagnosa_askep_luaran_nama')[$i],
          'diagnosa_askep_luaran_definisi' => $this->input->post('diagnosa_askep_luaran_definisi')[$i],
          'diagnosa_askep_luaran_kategori' => $this->input->post('diagnosa_askep_luaran_kategori')[$i],
          'diagnosa_askep_luaran_subkategori' => $this->input->post('diagnosa_askep_luaran_subkategori')[$i],
...
// echo json_encode($params); die;
      try {
        ...
        $response = $this->_client_rs->request('POST', 'askep_luaran', $this->_config);
