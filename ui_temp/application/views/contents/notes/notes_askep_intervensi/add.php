...
<form id="form-add">
  ...
  <div class="row">
    ...
    <!-- <div class="panel panel-primary">
            ...
            <div class="col-md-12">
              ...
              <div class="row">
                <div class="col-md-12">
                   ...
                   <div class="col-md-10">
                      <select name="askep_diagnosa" id="askep_diagnosa" style="width: 100%">
                        <option value=""></option>
      ...
      <div class="panel panel-primary">
        <div class="panel-heading">Notes Asuhan Keperawatan intervensi</div>
        <div class="panel-body">
          <div class="row">
            <div class="col-md-12">
              ...
              <div class="row">
                ...
                <div class="col-md-10">
                      <select name="askep_diagnosa" id="askep_diagnosa" style="width: 100%">
                        <option value=""></option>
                        <?php foreach ($data_askep_diagnosa as $v){?>
                          <option value="<?= $v['id'] ?>">
                            <?= $v['askep_diagnosa_kode'] ?> | <?= $v['askep_diagnosa_nama'] ?>
                          </option>
                        <?php }; ?>
                      </select>
                  </div>
                  ...
                  <br>&nbsp;
                  <div class="col-md-10">
                    <select name="askep_intervensi" id="askep_intervensi" style="width: 100%">
                      <option value=""></option>
                      <?php foreach ($data_askep_intervensi as $v){?>
                        <option value="<?= $v['id_ref_askep_intervensi'] ?>">
                          <?= $v['askep_intervensi_kode'] ?> | <?= $v['askep_intervensi_nama'] ?>
                        </option>
                      <?php }; ?>
                    </select>
                  </div>
                  <div class="col-md-2">         
                          <button class="btn btn-sm btn-primary btn-add" type="button">Tambah</buton>
                  </div> 
...
<script type="text/javascript">
  ...
  $('#askep_diagnosa').select2({
    placeholder: "-- Pilih Askep Diagnosa --"
  });

  $('#askep_intervensi').select2({
    placeholder: "-- Pilih Askep intervensi --"
  });
  ...
  $('.btn-add').click(function(e) {
    ...
    var as_intervensi = $("#askep_intervensi").val();

    $.ajax({
      type :'get',
      url:'<?php echo base_url(); ?>' + class_name + "/detail_intervensi/" + as_intervensi,
      dataType:'json',
      success : function(data){  
        var newRow = `<tr>
                    <input type="hidden" name="diagnosa_askep_intervensi_kode[]" value="` + data.askep_intervensi_kode + `">
                    <input type="hidden" name="diagnosa_askep_intervensi_nama[]" value="` + data.askep_intervensi_nama + `">
                    <input type="hidden" name="diagnosa_askep_intervensi_definisi[]" value="` + data.askep_intervensi_definisi + `">
                    <input type="hidden" name="diagnosa_askep_intervensi_kategori[]" value="` + data.askep_intervensi_kategori + `">
                    <input type="hidden" name="diagnosa_askep_intervensi_subkategori[]" value="` + data.askep_intervensi_subkategori + `">
                    <td><span class="sn"></span>
                    <td><button type="button" class="btn btn-sm btn-danger hapus">Hapus</button></td>
                    <td>` + data.askep_intervensi_kode + ` | ` + data.askep_intervensi_nama + `</td>
                  </tr>`
          $('#list_intervensi').append(newRow);
          $('#list_intervensi tr').each(function(index) {
            //alert(index);
          ...
          $("#askep_intervensi").val('').trigger('change');
        }
    });
  ...
  $(document).on('click', '.hapus', function() {
    ...
    $('#list_intervensi tr[trt="' + tr_value + '"]').remove();
  ...
  $('#form-add').submit(function(e) {
    ...
    $.post('<?php echo base_url(); ?>' + 'notes_askep_intervensi/add_process', $(this).serialize()).done(function(data) {
      var data = JSON.parse(data);
