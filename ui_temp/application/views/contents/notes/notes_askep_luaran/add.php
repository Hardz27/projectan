...
<form id="form-add">
  ...
  <div class="row">
    ...
    <!-- <div class="panel panel-primary">
      ...
      <div class="panel panel-primary">
        <div class="panel-heading">Notes Asuhan Keperawatan Luaran</div>
        <div class="panel-body">
          <div class="row">
            <div class="col-md-12">
              ...
              <div class="row">
                ...
                <div class="col-md-10">
                  ...
                  <br>&nbsp;
                  <div class="col-md-10">
                    <select name="askep_luaran" id="askep_luaran" style="width: 100%">
                      <option value=""></option>
                      <?php foreach ($data_askep_luaran as $v){?>
                        <option value="<?= $v['id_ref_askep_luaran'] ?>">
                          <?= $v['askep_luaran_kode'] ?> | <?= $v['askep_luaran_nama'] ?>
                        </option>
                      <?php }; ?>
                    </select>
                  </div>
...
<script type="text/javascript">
  ...
  $('#askep_luaran').select2({
    placeholder: "-- Pilih Askep luaran --"
  });
  ...
  $('.btn-add').click(function(e) {
    ...
    var as_luaran = $("#askep_luaran").val();

    $.ajax({
      type :'get',
      url:'<?php echo base_url(); ?>' + class_name + "/detail_luaran/" + as_luaran,
      dataType:'json',
      success : function(data){  
        var newRow = `<tr>
                    <input type="hidden" name="diagnosa_askep_luaran_kode[]" value="` + data.askep_luaran_kode + `">
                    <input type="hidden" name="diagnosa_askep_luaran_nama[]" value="` + data.askep_luaran_nama + `">
                    <input type="hidden" name="diagnosa_askep_luaran_definisi[]" value="` + data.askep_luaran_definisi + `">
                    <input type="hidden" name="diagnosa_askep_luaran_kategori[]" value="` + data.askep_luaran_kategori + `">
                    <input type="hidden" name="diagnosa_askep_luaran_subkategori[]" value="` + data.askep_luaran_subkategori + `">
                    <td><span class="sn"></span>
                    <td><button type="button" class="btn btn-sm btn-danger hapus">Hapus</button></td>
                    <td>` + data.askep_luaran_kode + ` | ` + data.askep_luaran_nama + `</td>
                  </tr>`
          $('#list_luaran').append(newRow);
          $('#list_luaran tr').each(function(index) {
            //alert(index);
          ...
          $("#askep_luaran").val('').trigger('change');
        }
    });
  ...
  $(document).on('click', '.hapus', function() {
    ...
    $('#list_luaran tr[trt="' + tr_value + '"]').remove();
  ...
  $('#form-add').submit(function(e) {
    ...
    $.post('<?php echo base_url(); ?>' + 'notes_askep_luaran/add_process', $(this).serialize()).done(function(data) {
      var data = JSON.parse(data);
